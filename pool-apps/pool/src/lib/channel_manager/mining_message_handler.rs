    ...
    async fn handle_open_standard_mining_channel(
        ...
        let messages = self.channel_manager_data.super_safe_lock(|channel_manager_data| {
            ...
            downstream.downstream_data.super_safe_lock(|downstream_data| {
                ...
                let mut standard_channel = match StandardChannel::new_for_pool(channel_id, user_identity.to_string(), extranonce_prefix.to_vec(), requested_max_target, nominal_hash_rate, self.share_batch_size, channel_manager_data.current_shares_per_minute, job_store, channel_manager_data.current_pool_tag.clone()) {
                    Ok(channel) => channel,
                    Err(e) => match e {
                ...
                downstream
                    ...
                    .super_safe_lock(|downstream_data| {
                        ...
                        let mut extended_channel = match ExtendedChannel::new_for_pool(
                            ...
                            channel_manager_data.current_shares_per_minute,
                            job_store,
                            channel_manager_data.current_pool_tag.clone(),
                        ) {
                            Ok(channel) => channel,
                            Err(e) => match e {
                ...
                match res {
                    Ok(ShareValidationResult::Valid(share_hash)) => {
                        ...
                        let payload = crate::webhook::ShareWebhookPayload {
                            user_id,
                            share_hash: format!("{:x}", share_hash),  // Hex format
                            difficulty: extended_channel.get_target().difficulty_float(),
                            job_id: msg.job_id,
                            channel_id: msg.channel_id,
                            downstream_id,
                            sequence_number: msg.sequence_number,
